...
2. **Train** — in [`ml/`](ml/): `pip install -r requirements.txt` then
   `python train.py --data ./data --epochs 60 --export` (PyTorch GRU).
3. **Deploy** — the export writes `public/violence.onnx`; reload the app and
   [classifier.ts](lib/classifier.ts) auto-detects and runs it.

See [ml/README.md](ml/README.md) for details and the public-dataset upgrade path.
